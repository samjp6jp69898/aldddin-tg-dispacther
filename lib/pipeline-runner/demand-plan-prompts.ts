...
- 重點先行：每個章節先一句結論再展開細節；用詞白話，內部代號（server／method／元件名）第一次出現時帶半句說明，不要假設讀者跟過這條 pipeline。
...
## 需求摘要
（3–5 句白話：這張單要做什麼、為什麼、會動到哪些 repo／模組。最後固定兩行：「影響範圍：...」「風險與待確認：...」，沒有就寫「無」——讓沒看過需求單的人 30 秒抓到重點）
