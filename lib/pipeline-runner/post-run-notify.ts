...
async function main(): Promise<void> {
  ...
  // 註解）：本檔（bug pipeline 的 EXIT trap 短命 CLI，spawn 端見
  // spawn-create-mr.ts）不是固定只在 head 機器上跑——lib/cluster/dispatch.ts
  // 的派工機制會把 bug pipeline 派去 worker 執行，跟 demand pipeline 同一套
  // 機制。在最早執行處依本機 `.env` 的 MON_DB_USER 宣告對應角色，不用猜——
