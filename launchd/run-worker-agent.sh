...
# .env 逐一匯出（2026-08-31 前是根目錄 .env），token/secret 不出現在腳本或 plist 明文裡。
#
# 需要的 .env 變數（worker 機的 telegram-dispatcher/.env）：
...
#                          與 head 同值
# 另外 /create-mr pipeline 本身需要的其他變數（Notion token 等）走 aladdin_ai/
# .env.* 那條線，不在這份 telegram-dispatcher/.env 裡——worker 機要另外準備
# aladdin_ai 的 .env.local 等檔案（直接從 head 安全複製一份即可，AirDrop/scp/USB，
# 不走會落地存放的通道）。
set -u
