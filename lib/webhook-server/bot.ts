...
// 只透過 process.env 讀取，值本身來自 telegram-dispatcher/.env（2026-08-31 前為根目錄 .env）（啟動 wrapper
...
if (!token) {
  throw new Error('TG_DISPATCH_BOT_TOKEN is required (export it from telegram-dispatcher/.env before starting)')
}
