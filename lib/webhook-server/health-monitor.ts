...
// metrics server 預設監聽 127.0.0.1:20241（實測見 commit 說明），/ready 是它內建
// 的健康檢查端點，回傳 { status, readyConnections, connectorId }——比 ngrok 的
...
/**
 * 唯讀查詢本機 cloudflared metrics server 的 /ready 端點，判斷 tunnel 是否還連著
 * 邊緣節點。查不到／逾時／回應格式不對都當作不健康，不拋例外——健康檢查本身
 * 出錯不該連帶讓 process 掛掉。readyConnections 是目前註冊成功的邊緣連線數，
 * 正常情況下是 4（見 cloudflared 實際輸出），0 代表雖然行程活著但完全連不上
 * Cloudflare（跟 ngrok 版「查得到 API 但 tunnels 是空陣列」是同一種故障語意）。
 */
export async function checkCloudflaredTunnelReachable(apiUrl: string = CLOUDFLARED_READY_URL): Promise<boolean> {
  try {
 ...
 * 存活，不保證 tunnel 真的還連著』——這裡直接查 cloudflared 本機 metrics
 * server 的 /ready 判斷 tunnel 是否還在，只在『健康 ↔ 不健康』狀態真的
 ...
 * 去規避某個競態。
 *
 * 2026-08-22：改用 cloudflared 取代 ngrok（使用者裁定，H28 risk_notes (12)
 * 收斂）——cloudflared 沒有 ngrok 免費方案「同帳號同時只允許 1 個 tunnel
 * session」的限制，但 KeepAlive 只保證 process 存活、不保證 tunnel 真的還
 * 連著這個風險本質不變，檢查邏輯原樣沿用，只換底層查的 API。
 *
 * 已知範圍缺口（review 發現，非本次修復範圍）：這只覆蓋『tunnel 斷線』這
 * 一半，沒有覆蓋「bun server 本身 event loop 卡死」那一半——若真的整個卡
 * 死，這裡的 setInterval callback 本身也不會觸發，是「靜默不再檢查」而非
 * 「主動偵測到自己掛了並告警」。這種自我不可觀測的故障，結構上只能靠外部
 * （另一個獨立 process）定期從外部戳 /health 才能偵測，不是這支「輕量」
 * 內部檢查的範圍，需要的話應該另開一個 task。
 */
export function createHealthMonitor(
  ...
  const apiUrl = deps.apiUrl ?? CLOUDFLARED_READY_URL
  ...
  // 跟 tunnel 那半刻意不同：這裡**第一次檢查就會告警**，沒有「只記基準值」的
  ...
  async function runOnce(): Promise<boolean> {
    ...
    if (healthy !== lastKnownHealthy) {
      ...
      const text = healthy
        ...
        : '⚠️ [dispatcher 健康檢查] Cloudflare tunnel 偵測不到（本機 20241 metrics/ready 打不到或沒有 ready connection），Telegram webhook 可能已經收不到訊息，請檢查 cloudflared 行程是否還活著'
      notify(text)
    }
    return healthy
  }
