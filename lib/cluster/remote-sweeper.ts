...
//   already_running_remote 擋住；只告警一次，等 worker 恢復或 26 小時
//   絕對上限。清了反而讓單子回到可認領池。
// - 只有「向 worker 查證確認沒有任何活動」或「超過 26 小時絕對上限」才清。
...
//   只有「確定沒有任何流程碰過這張單」（dispatching 求證全空）與 26 小時
...
export const SWEEP_MAX_AGE_MS = 26 * 3600_000 // 絕對上限：worker 端排隊最長 24h + 單輪 timeout 2h
    ...
    for (const entry of deps.registry.list()) {
      ...
      if (entry.status === 'dispatching') {
        ...
        if (results.some(r => r.status === null)) {
          // 有 worker 失聯：可能就在那台上，保留登記等下一輪（受 26h 上限）。
          continue
        }
      ...
      if (status === null) {
        ...
        if (state.misses >= SWEEP_MISS_LIMIT && !state.alerted) {
          ...
          // 登記保留即持續擋住重複認領；告警一次，等恢復或 26h 上限。
          state.alerted = true
